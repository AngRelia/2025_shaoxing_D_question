...
{
	/* 由SysConfig初始化I2C引脚，这里仅等待OLED上电稳定 */
	delay_us(10000);
}
...
static void OLED_I2C_Transmit(const uint8_t *Data, uint8_t Count)
{
	while (!(DL_I2C_getControllerStatus(I2C_OLED_INST) & DL_I2C_CONTROLLER_STATUS_IDLE));
	DL_I2C_fillControllerTXFIFO(I2C_OLED_INST, (uint8_t *)Data, Count);
...
{
	uint8_t TxBuf[2] = {0x00, Command};
	OLED_I2C_Transmit(TxBuf, 2);
}
